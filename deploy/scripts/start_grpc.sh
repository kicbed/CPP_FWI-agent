#!/bin/bash
#
# FWI Agent gRPC 客户端启动脚本
#
# 功能：
# 1. 启动 Embedding 服务
# 2. 启动 Agent 系统（Orchestrator 监听 :5000）
# 3. 启动 gRPC Client（直连 Orchestrator HTTP API）
#
# 注意：gRPC Client 底层是 HTTP POST (curl)，直接连接 Orchestrator 的
#       A2A JSON-RPC 接口 (port 5000)，不是 gRPC 协议。
...
echo -e "${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════════╗"
echo "  ║           FWI Agent 客户端启动                              ║"
echo "  ╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

# 加载 .env
if [ -f "$PROJECT_ROOT/.env" ]; then
    echo -e "${GREEN}[1/3] 加载配置...${NC}"
    set -a
...
# 启动 Embedding
echo -e "${YELLOW}[2/3] 启动 Embedding 服务（首次加载模型约需 15 秒）...${NC}"
...
# 启动 Agent 系统
echo -e "${YELLOW}[3/3] 启动 Agent 系统...${NC}"
...
# 启动 gRPC Client（前台，直连 Orchestrator HTTP API）
echo -e "${YELLOW}启动 gRPC 客户端...${NC}"
echo ""
"$PROJECT_ROOT/build/client/grpc_ai_client" http://localhost:5000
