...
public:
    AIOrchestrator(const std::string& agent_id,
        ...
        , agent_retriever_(registry_client_, orch_config.dashscope_api_key)  // Pass DashScope API key for embedding
...
std::string analyze_intent(const std::string& text) {
    ...
    try {
        ...
        std::cout << "[Orchestrator] 原始意图识别结果: " << result << std::endl;
        ...
        std::cerr << "[Orchestrator] 意图识别失败，使用 general 兜底: "
                  << e.what() << std::endl;
        return "general";
    }
}
    ...
    std::string handle_general_query(const std::string& query, const std::string& context_id) {
        try {
            ...
            std::cerr << "[Orchestrator] general 问答失败: "
                    << e.what() << std::endl;
    ...
    std::string handle_fwi_query(const std::string& query, const std::string& context_id) {
        try {
            ...
            std::cerr << "[Orchestrator] FWI 问答失败: "
                    << e.what() << std::endl;
