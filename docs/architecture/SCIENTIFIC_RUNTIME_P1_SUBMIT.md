...
后续 P2-004 没有改变上述 P1 submit/exact-replay 语义，而是增加了一个独立、仅在 loopback
Workbench 组合启动时运行的有界恢复入口。它先从 SQLite 按 scope 全分页读取 active task，
但不 claim/首次派发 pending，也不把普通 dispatch 当作 dispatching probe。它只通过 intent
推导的 current 1.4 Adapter record 做无副作用 lookup；私有状态已 durable `launched` 且 exact
receipt 通过严格校验时才写唯一 outcome，并做一次 status/event 追赶。missing/no-record、
preparing/launching/failed/corrupt 保持 deferred，`reconciliation_required` 仍不重试，也没有
fenced capacity、lease/heartbeat、持续 supervisor、cancel、retry 或 SSE。

后续 P2-005A 已增加 SQLite v8 scope-level fenced **控制面** lease，以及只观察既有
`dispatched` task 的持续状态泵；它不改变本节的 submit/exact-replay、pending/no-record 和
`reconciliation_required` 边界。该 lease 不是 Worker capacity/attempt lease 或 Worker
heartbeat，Supervisor 也没有 dispatch/launcher 能力。
...
结果展示已由 P1-005 验证。当前仍 Pending：完整 P2 的 Worker capacity/attempt lease 与
heartbeat、staged launch、pending/no-record 调度、cancel/retry/reconciliation/SSE、P3 DAG，
以及 P4 Agent Planner。
P2-004 只把“Adapter 已 durable launched、SQLite outcome 丢失”这一 dispatching 子窗口做成
有界 Verified；pending、claim 后尚无 record 与 launch outcome 不明确的窗口仍 Pending，不能把
本段的完整 P2 列表改写为已完成。
