...
中等、纵向、可独立验证的切片：共享同一状态机、接口和出口测试的相邻工作合并；出现明确
并发/崩溃窗口、独立安全边界、文件所有权冲突或无法共同验证的风险时，必须先说明证据并获得
用户明确批准，随后才能拆分并记录理由。不得为单个 migration、字段、receipt/sidecar 或一项
测试分别创建产品路线切片，也不得为了减少编号把高风险工作压成不可审阅的大提交。D-011 在
`0cbe131` 的“约十余个”是覆盖当时完整 P2 余项
及 P3–P6 的历史粗估基线，不是当前余量或单个 P 级别配额；滚动余量只以进度账本为真源。
