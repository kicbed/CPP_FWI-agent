...
阶段和出口条件继续作为质量边界。路线切片是弹性的产品交付/估算单位；工作轮次是一次 Codex
请求内的有界执行单位，只含一个主要验收目标和一个风险边界，可以在路线切片仍 `In progress`
时安全交接。裸命令“继续 D-003”只推进一个工作轮次，不承诺一次完成整个切片或阶段；工作轮次
不新增路线编号，也不改变滚动余量。共享同一状态机、接口和出口测试的相邻工作仍合并为路线
切片；新增路线切片须先说明具体证据并获用户批准，不得按 migration、字段或单项测试机械拆分。
D-011 在 `0cbe131` 的“约十余个”是覆盖当时完整 P2 余项及 P3–P6 的历史粗估基线，不是当前余量或单个 P 级别配额；当前余量只以进度账本为真源。

验证固定为三级：工作轮次内只跑受影响/失败测试；路线切片出口在候选最终 tree 上只跑一次相关
集成 aggregate；阶段出口才跑完整回归和适用的代表性 CPU/CUDA/E2E。候选 tree 后续变化时先补
受影响测试，再对新的最终 tree 跑一次 aggregate。共享执行/安全合同变化只有在具体证据表明更广
suite 失效时才扩大验证，并在运行前说明触发原因；不得自动上升为全仓回归。数值 Worker、依赖、
数据和规范化配置身份不变时复用绑定 Git tree、环境、数据和配置的既有实验。每轮默认一次综合
审阅；第三轮及以后独立审计必须先获用户明确批准。aggregate 只报告一次，已包含子集不重复相加。
...
user cancel 已由 P2.7 关闭，独立 **P2.8 exact-attempt wall-time timeout** 也已 Verified。P2.8 终态
固定为 `Failed / WALL_TIME_EXCEEDED`；clock 精确从 Store 对 current Worker 的首条 SQLite durable
`spawned + ready + running` observation 的 `observed_at` 开始。deadline 到来且 active term 持久化
authorization 前 Worker mutation 为零；自然终态在 authorization 前完成为 `not_triggered`，durable
user-cancel admission 先赢为 `suppressed`，authorization/request 后自然终态先赢为 `superseded`，
完整 request + ack + stopped heartbeat + idle execution fence proof 才是 `timed_out`。grace 后由
exact Worker 自行退出，控制面绝不根据持久 PID 发送 signal。该能力只对 current 1.4/private 1.1
且能证明 v2 exact-stop capability 的 managed attempt 开放；Workbench 仅投影七个 bounded timeout
字段且无 mutation。

当前 **P2.9A 有界 positive receipt resolution/adoption** 已 Verified。它只允许 active fenced
Supervisor term 对 immutable `reconciliation_required` intent 的两类 current Adapter `1.4.0`
严格正向证明追加 resolution/adoption：current managed attempt 的 spawned + exact ready + heartbeat
（`running`/`succeeded`/`failed`），以及
current Adapter 下 legacy-private schema `1.0` 的 exact launched receipt。
公共 Adapter `1.0`–`1.3` 不进入该边界；缺失、歧义、损坏、不匹配、部分、launch/ticket failed
或 staged evidence 保持 `action_required`。spawned + exact ready 后的 terminal heartbeat
`succeeded`/`failed` 仍是正向 receipt，认领后同周期执行 status catch-up。原 outcome 不改写或删除，
...
reconciliation、SSE 与完整 P2 故障/CPU/CUDA 出口验收。这是当前规划基线而非固定配额；出现
具体安全或验证证据时仍须先获得用户明确批准，批准后才能拆分并把理由写入进度账本，且不得
降低上述完整 P2 出口条件。`resources.wall_time_seconds` 的
...
安全边界时必须先说明证据并获用户明确批准，随后才能拆分并记录；该估算不是配额。
...
- 本计划已经获批，后续 Codex 不得修改或重新解释其范围、阶段顺序、依赖、安全边界或退出标准；
  任何变化都必须先获得用户明确同意。
- 完整项目固定为 P0→P1→P2→P3→P4→P5→P6；只有 P6 出口通过才完成，任何摘要不得以 P5 收尾。
- 新建议先说明证据、收益、成本、风险和兼容性；如涉及任何编号 D 条目，必须先展示单 D 的
  精确 diff/SHA-256 并停止，只有用户随后单独原样复制 Codex 给出的唯一 `D-AUTH` 句才可修改。
- “同意/继续/固定/记录/修正”均不授权 D 变更；历史 D-001～D-012 保持原状，普通状态只写进度账本。
- 仅执行进度、测试结果和阻塞变化时更新 `docs/PROJECT_PROGRESS.md`，不重写已批准目标。
- 所有数量、工期和剩余切片估算都必须标明全项目/阶段/子切片范围、包含阶段，以及粗估或承诺
  性质；不得用当前阶段子集替换整个项目估算。
- 当前对话中的最新明确指示除最高优先级 D 锁外优先；直接要求改 D 仍只触发提案，不能代替原样授权句。
...
| 2026-07-16 | 接受并开始 P2.8 exact-attempt wall-time timeout：Failed/WALL_TIME_EXCEEDED、首条 durable ready+running observation 起算、grace 后 Worker 自退出、timeout delivery authorization 与 durable cancel admission first-writer-wins | 用户明确要求按建议实现 timeout；保持控制面不 signal 持久 PID并限定 current 1.4/private 1.1 + v2 exact-stop capability |
...
| 2026-07-17 | 纠正 D-011 估算口径：`0cbe131` 的历史基线覆盖完整 P2 余项与 P3–P6；当前滚动余量由进度账本随 Verified 切片递减 | 用户明确要求不能把旧“十余个”原数带到项目末期；P2 当前两个剩余切片只是全项目子集 |
| 2026-07-17 | 撤销 `419c41f` 中未经授权新增的决策编号；估算规则归入 D-011，计划变更与编号授权归入既有 D-002/D-003 | 用户明确说明“固定”不等于允许 Codex 自行增加 D 编号；阶段顺序、内容和质量门未改变 |
| 2026-07-17 | 修订 D-011 的永久默认接续：路线切片与有界工作轮次分离，轮次内聚焦测试、切片出口单次 aggregate、阶段出口完整回归/代表性 CPU-CUDA，并限制审计轮次和短入口体积 | 用户要求永久改变“继续 D-003”的默认语义，并保证 P3–P6 不再重复出现单次任务臃肿、重复测试和高 token 上下文问题；阶段、出口质量和滚动余量不变 |
