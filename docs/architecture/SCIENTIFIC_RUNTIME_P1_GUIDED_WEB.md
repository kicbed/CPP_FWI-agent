...
权威是 Worker 继承的 kernel `flock`，heartbeat 只是证据，不授权接管。运行中取消、
timeout、通用有限 task retry、自动 reconciliation 闭环和 SSE 仍未实现，也没有
...
`continuous_status_supervision=true`、`supervisor_leases=true`；
...
cancel、timeout、通用有限 task retry、自动 reconciliation 解决或 SSE 已实现。
...
13. `./stop.sh` 后重新启动，左栏应重新发现原 task，重开后能读到相同终态、已落库事件和
    ...
    takeover。这仍不是 cancel、timeout、通用 task retry、完整 reconciliation 或 SSE 证据。
...
同 attempt 安全 resume、收养与状态投影。这不改写上述历史 checkpoint 证据，也不把 partial staging 修复、
reconciliation 解决、cancel、timeout、通用有限 task retry 或 SSE 误报为已完成。
