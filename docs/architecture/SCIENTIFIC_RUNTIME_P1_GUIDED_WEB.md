...
- 后续维护：`D-007 / P1-007 / P2-001` **Verified**；`D-008` 的对话/任务分离、
  ...
  受监督调度，`P2-007` 已实现有界 exact-attempt user cancellation，`P2-008` 已实现有界
  exact-attempt wall-time timeout；完整 P2 仍 Pending
...
managed Worker 提供用户取消；P2-008 已为具备 v2 exact-stop capability 的同类 Worker 提供自动
wall-time enforcement。通用有限 task retry、自动 reconciliation 闭环和 SSE 仍未实现，也没有
...
`features.runtime_timeout=true`、`capabilities.cancel=true`；timeout 是自动控制面能力，不存在
`can_timeout`、`capabilities.timeout` 或 timeout mutation。
...
通用有限 task retry、自动 reconciliation 解决或 SSE 已实现。
...
13. `./stop.sh` 后重新启动，左栏应重新发现原 task，重开后能读到相同终态、已落库事件和
    ...
    takeover。P2-007 只为 exact current managed attempt 增加 user cancel，P2-008 只为具备 v2
    exact-stop capability 的该类 attempt 增加自动 timeout；这仍不是通用 task retry、完整
    reconciliation 或 SSE 证据。
...
15. 同一详情页只读显示 timeout 的 `state`、`wall_time_seconds`、`started_at`、`deadline_at`、
    `resolved_at`、`failure_code`、`terminal_status` 七个字段，不显示 ID/hash/PID/path，也没有
    timeout 按钮或 `POST /timeout`。clock 从 Store 首次持久观察 current Worker
    spawned+ready+running 的 `observed_at` 起算；authorization 前自然终态为 `not_triggered`，cancel
    admission 先赢为 `suppressed`，authorization/request 后自然终态先赢为 `superseded`，只有
    request + ack + stopped heartbeat + idle fence 全证据才显示 `timed_out` 与
    `Failed / WALL_TIME_EXCEEDED`。
...
只允许用户复用原 key 显式继续。P2-008 已验证独立 exact-attempt wall-time enforcement：它只覆盖
current 1.4/private 1.1 且能证明 v2 exact-stop capability 的 managed attempt，从 Store 首条
current Worker durable ready+running observation 的 `observed_at` 起算，完整停止 proof 后投影
`Failed / WALL_TIME_EXCEEDED`；Workbench 只显示七个 bounded 字段且无 timeout mutation。固定 venv
CPU Deepwave timeout E2E 与 Runtime 304/304、Worker 32/32、launch-control 25/25、Web 46/46、
Embedding 6/6、CTest 39/39、MCP 1/1、Node/governance 均 PASS，未运行 CUDA。关闭卡片、放弃草稿
和 Trash 仍是不同操作。这不改写上述历史 checkpoint 证据，也不把 partial staging 修复、
reconciliation 解决、通用有限 task retry 或 SSE 误报为当时已完成；旧任务和旧 Worker 仍不受
timeout 能力覆盖。
