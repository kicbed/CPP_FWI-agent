...
dispatched task 做一次 status/event 追赶。成功后才 activate/publish/serve；忙端口零 recovery。
它不扫描运行目录、不调用 launcher、不依赖浏览器点击；持续 supervisor、fenced capacity、
lease/cancel/retry/完整 reconciliation/SSE 仍不处理。
...
13. `./stop.sh` 后重新启动，左栏应重新发现原 task，重开后能读到相同终态、已落库事件和
    ...
    outcome 丢失的 exact receipt，并追赶一次状态；pending/no-record/ambiguous 不派发。这仍
    不是 fenced capacity、lease、持续 reconciliation、cancel、retry 或 SSE 证据。
...
P2-004 当前有界证据为 TaskService 80/80、Adapter 27/27、Scientific Runtime 201/201 与 Web
36/36；它覆盖 pending 零派发、真实固定 Adapter 的 launched-lost-receipt 零重启 Worker 收养、
missing/ambiguous/corrupt deferred、严格并发 handle、
跨页/scope/limit、1000+ events、终态追赶、bind/忙端口/失败清理和脱敏 summary，未运行真实
FWI/CUDA。仍属 Pending：运行中 cancel、timeout、fenced capacity、pending/no-record 调度、
lease/heartbeat、retry、完整自动 reconciliation、SSE、启动 pass 后不依赖用户点击的持续事件
推进、DAG、Agent Planner、通用算法 SDK。Adapter provenance 仍是 development，不据此声明
跨环境 bitwise 可复现或真实数据上的科学效果。
