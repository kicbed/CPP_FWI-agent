...
- 后续维护：`D-007 / P1-007 / P2-001` **Verified**；`D-008` 的对话/任务分离、
  ...
  `P2-004` 启动 receipt 收养与 `P2-005A` 控制面持续状态泵已验证；完整 P2 仍 Pending
...
增加 scope-level fenced 控制面 lease；Worker capacity/attempt lease、Worker heartbeat、取消、
重试、自动 reconciliation 和 SSE 仍未实现，也没有 P3 DAG 或 P4 Agent Planner。
...
7. Succeeded 后按该任务持久 Plan 精确收集输出：历史 1.0–1.3 任务仍为 NPY + CSV 两项；
...
后续 P2-004 在 loopback socket 已 bind 但尚未 listen 时执行一次 scope-bound 启动 pass：最多
扫描 10000 个 active task。pending 不 claim/首次派发；dispatching 只读查询 intent 推导的
current 1.4 Adapter record，只收养 exact `launched` receipt；其余状态 deferred。已有/新收养
dispatched task 做一次 status/event 追赶。P2-005A 随后在 listen/publish 前取得 SQLite v8 的
project/principal scope lease 并启动 observation-only Runtime Supervisor；ready 后才 activate、
publish/serve，忙端口或 lease 被另一 owner 持有时 API 都不会发布。Supervisor 只持续观察
durable outcome 精确为 `dispatched` 的 Queued/Running task，写入由连续 fencing term 约束；
pending/dispatching/missing/`reconciliation_required` 全部 deferred。它不扫描运行目录、不调用
launcher、不依赖浏览器点击。session capability 中
`continuous_status_supervision=true`、`supervisor_leases=true` 只表示这条控制面状态泵；
`startup_dispatch_recovery=false` 与 `automatic_reconciliation=false` 保持不变，不表示 Worker
fenced capacity/attempt lease、Worker heartbeat、cancel/retry/完整 reconciliation 或 SSE。
...
13. `./stop.sh` 后重新启动，左栏应重新发现原 task，重开后能读到相同终态、已落库事件和
    artifacts。P2-004 还会在 API 对外可用前收养 Adapter 已 durable `launched` 但 SQLite
    outcome 丢失的 exact receipt，并追赶一次状态；P2-005A 随后持有 fenced 控制面 lease，
    即使没有浏览器 GET 也持续追赶已有 dispatched task。pending/no-record/ambiguous 不派发。
    这仍不是 Worker fenced capacity/attempt lease、Worker heartbeat、完整 reconciliation、
    cancel、retry 或 SSE 证据。
...
P2-004 与 P2-005A 当前有界证据为 TaskService 80/80、Adapter 27/27、Scientific Runtime
...
未运行真实 FWI/CUDA。仍属 Pending：运行中 cancel、timeout、Worker fenced capacity/attempt
