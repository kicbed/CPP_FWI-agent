...
- PlanGraph 1.2 已能 hash-bound typed upstream node/output→downstream input，纯 binding 会复核真实
  artifact bytes/hash/size、source task/node/plan/algorithm/port/type、静态 Dataset lineage 与 fingerprint；
  ArtifactManifest 1.0 无法表达 chained artifact input 时 fail closed。durable 全输入聚合、producer
  `Succeeded` revision/receipt、approval/current term 原子围栏，以及可执行状态转换/派发仍 Pending。
...
- P2-006 已让 Web 进程在 scope-level fenced term 下调度并持续更新 task，不依赖浏览器 GET；
  ...
  all-input binding、可执行 node transition/dispatch/资源锁/cache/Recipe、P4 Agent Planner、P5 Algorithm SDK
  与 P6 加固仍未实现。
