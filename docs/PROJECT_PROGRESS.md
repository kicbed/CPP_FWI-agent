...
- 最后更新：2026-07-15
- 活跃决策：`D-003`、`D-004`、`D-006`、`D-007`、`D-008`；`D-005` 仍为 Proposed
- 活跃分支：`feature/scientific-agent-runtime`
- 基线：`feature/fwi-deepwave-2d-acoustic@ffeb5bc`
- 总体状态：**P0 + P1（含 P1-008）已验证；P2-001 有界发现/重开已验证；P2-002 有界
  回收站已验证；完整 P2 Pending**
- 当前阶段：**D-008/P1-008/P2-002 已验证；向用户交付实际试用/验收说明**
- 下一动作：交付前端试用教程、启动命令、验收请求和预期结果，并停在完整 P2 之前；不启动
...
| P2 持久可靠性加固 | In progress（P2-001、P2-002 Verified；完整 P2 Pending） | P2-001 scope-bound 分页发现/重开；P2-002 为 resolved terminal task 增加 append-only、CAS/idempotent Trash/Restore，保留审计与 artifact | fresh v6 active/trash、artifact read、revision 0→1→2、restore 不重跑、AwaitingApproval 409、abandon 后 trash 和同库重启 PASS | lease、取消、重试、reconciliation、SSE 及完整刷新恢复通过 |
...
- P2-002 的“删除任务”只是可恢复的 visibility Trash/Restore，不物理删除 Draft、Plan、
  Approval、RunEvent、幂等记录或 artifact；服务器 transcript 永久删除与 artifact 永久 purge
  仍未实现。
...
P2-002 可恢复任务回收站均已 Verified；这两个有界切片都不包含运行中
cancel、timeout、lease/heartbeat、task retry、自动 reconciliation、SSE 和不依赖用户点击的
恢复。服务器 transcript 永久删除和 artifact 永久 purge 也仍 Pending。D-005 仍未获批，没有
迁移或删除旧 prompt-like 文件。
