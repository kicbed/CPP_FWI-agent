...
- 总体状态：**P0、P1、P2 均已验证；P2-001～P2-009B2、reconciliation 矩阵、checkpoint /
  ...
  blocking、Task 聚合、active-term 重启/取消恢复及 inherited CPU/GPU `flock` 容量围栏；node
  cache/checkpoint、Recipe/Guided 与阶段出口仍 Pending。P3 In progress，P4–P6 Pending；只有
  P6 出口通过才算全项目完成**
- 当前阶段：**P3 确定性 DAG In progress；阶段出口尚未满足**
- 下一动作：另开 P3 node cache/checkpoint 工作包；不混入 Recipe/API/UI 或 P3 阶段出口，公开
...
| P3 确定性 DAG | In progress | readiness；v18 node/claim；PlanGraph 1.2 binding；v19 all-input binding；v20 单节点内核；v21 historical per-node intents、deterministic per-Task 串行多节点调度、durable descendant blocking、Task 聚合、active-term 重启/取消恢复与 inherited CPU/GPU `flock` 容量围栏 | 第 1～5 轮见下；第 6 轮 aggregate 一次运行 516 项：511 PASS、5 个陈旧 v21 upgrade fixture/assertion 失败，修正后仅重跑失败子集 5/5 PASS；CPU 最大观测并行 2、单 GPU 最大 1；`py_compile`/治理 PASS | node cache/checkpoint、固定 Recipe/Guided 与 P3 阶段出口通过 |
...
- P2-006 已让 Web 进程在 scope-level fenced term 下调度并持续更新 task，不依赖浏览器 GET；
  ...
  checkpoint / Waiting / resume 与只读 SSE 已 Verified，但跨进程 checkpoint restart、P3 node
  cache/checkpoint/Recipe、P4 Agent Planner、P5 Algorithm SDK
  与 P6 加固仍未实现。
