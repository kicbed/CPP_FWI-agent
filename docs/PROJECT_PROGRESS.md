...
- 下一动作：P4 Agent Planner 保持 Pending；只有后续用户明确要求继续时才进入 P4
- 交付粒度：D-011 在 `0cbe131` 的全项目 P2–P6 历史粗估基线约 12 个；此后 9 个切片已
  Verified，当前滚动粗估约 3 个，P2/P3 为 0、P4–P6 合计暂估约 3 个；均非固定配额
...
| 2026-07-20（P3 阶段出口） | 当前完整 P3 余项 + P4–P6 | 1：固定 Recipe/Guided + 完整 P3 阶段出口 | 9 | 无 | 约 3 个；P2/P3 = 0，P4–P6 合计暂估约 3；阶段内容和质量门不变 |
...
- PlanGraph 1.2 已能 hash-bound typed upstream node/output→downstream input；SQLite v19 durable fact
  ...
  允许同一 Task 保存历史 exact per-node intent，并为所有历史/普通 DAG 保留每 Task 最多一个 active
  node；SQLite v23 只对服务端固定 Recipe 放行 B/C 两个 active admission。D 严格等待 B/C 均成功，
  跨 Task Worker 并发继续受 inherited CPU/GPU `flock` 约束。
...
- DAG 控制面恢复、node cache、Worker checkpoint 是三个不同边界。v22 只允许 exact 当前 Running
  ...
  producer semantic key、receipt/lineage hash 与 transitive Dataset roots，可从 E→D→B/C→A 重建到
  原始 Dataset。固定 Recipe 的 upstream artifacts 是真实 bytes/hash/receipt 校验的控制面输入和 cache /
  lineage 身份，不扩张为 P5 动态 Algorithm 输入或通用自动算法流水线。
- P3 固定 Recipe `forward_qc_fwi@1.0.0` 只有显式选择时才生成
  `A data_check → {B forward, C quality_check} → D fwi → E result_check`。同一 PlanGraph 经确认页展示
  node/dependency、Algorithm/Adapter 1.6、参数/资源/风险/输出与 `plan_hash`；任何节点、依赖或参数变化
  都使旧 Approval 失效。生产 API/UI/SSE 投影 per-node dependency/resource wait、cache/checkpoint、失败和
  lineage；公开事件删除 intent/attempt/path。普通 Guided FWI、仅注册多个算法及历史 PlanGraph 仍单节点，
  `dag=true` 只由完整 `web/serve.py` 生产组合启用。
- P3 阶段出口的 fresh CPU HTTP/SSE E2E 证明 A 完成后新 composition 不重复派发（1→1）、artifact
  hash 不变、B/C 真实重叠且 D/E 严格等待、SSE 1–30 连续、8 个公开 artifact finite/hash/lineage
  正确；五节点合法 cache hit 均零 dispatch，参数变化全 miss，源 bytes 篡改在目标零 dispatch 时以
  `ADAPTER_ARTIFACT_INVALID` fail closed。fresh CUDA 双 Workflow 均 Succeeded，单 GPU 最大真实并行 1。
...
- D-011 已批准：保持阶段、依赖、测试和安全质量；路线切片与有界工作轮次分离，裸“继续 D-003”
  每次只推进一个轮次。当前全项目路线切片粗估约 3、P2/P3 为 0；轮次不进入余量公式。多算法是
  独立可选工具，自动串联不作为当前出口。
...
- Deepwave Adapter 仍是固定 `acoustic_fwi_2d` 产品实现，不是 P5 通用 Algorithm SDK。P3 Recipe
  的上游 artifact 参与真实 bytes/hash/receipt、调度、cache 与 lineage 验证，但不是 Algorithm 1.6
  的动态数值参数；E 在验证 D 的 model/loss receipt 后运行固定 forward diagnostics，不宣称生成
  新的反演模型。cache-hit target 的 bytes 由 source run 物理持有；target purge 不删 source，但
  source 先永久 purge 后 target bytes 的可用性不在 P3 保证内；
- P2-006 已让 Web 进程在 scope-level fenced term 下调度并持续更新 task，不依赖浏览器 GET；
  ...
  checkpoint / Waiting / resume 与只读 SSE 已 Verified；P3 node cache/可信 lineage、DAG same-live
  checkpoint 和固定 Recipe/Guided 均已 Verified，但跨进程 checkpoint restart、P4 Agent Planner、
  P5 Algorithm SDK 与 P6 加固仍未实现。
