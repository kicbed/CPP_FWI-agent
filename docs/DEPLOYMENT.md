...
| 内容 | 路径 | 访问方式 |
|---|---|---|
| 仓库 | `/root/projects/project/agent-communication-main-v2` | 源码与构建 |
...
进程命令行。推荐把 secret 放在仓库外、权限为 `0600` 的文件中：
...
- MCP FWI runner 只接受固定 model/preset/device，使用固定 argv 启动 Worker；
