...
> **历史归档说明（2026-07-14）：** 本目录记录 2026-06 的 v0.2–v1.0
> review-only 演进过程，不是当前安装、启动或能力说明。当前固定白名单
> `marmousi_94_288` 路径已经能通过 Deepwave 在 CPU 或单张 NVIDIA GPU 上执行真实
> 正演和实验性 FWI；通用 `JobBackend` 仍保持 dry-run，MPI、多 GPU、SSH、Slurm/PBS、
> 远程集群和任意 shell 仍未开放。当前用法以仓库根目录
> [README](../../README.md)、[部署说明](../DEPLOYMENT.md)、
> [模型指南](../MODEL_GUIDE.md) 和 [前端测试教程](../FRONTEND_TEST.md) 为准。
> 本目录后续出现的 “No real CUDA/MPI” 等表述只描述对应历史版本或通用后端，
> 不能用来判断当前 Deepwave FWI 白名单路径。

This directory preserves the historical upgrade record that led to the current
workbench. Keep it for design context; do not use it as the current runbook.

## Historical Target and Timeline
...
- At that historical v0.2 point, no real CUDA/MPI or cluster execution existed.
...
`v0.14 Approved Template Run Packet` 已在 2026-06-23 完成第一批实现：

- `docs/upgrade/test-report-v0.14.md`
- `docs/upgrade/learning-summary-v0.14.md`
- 新增 `ApprovedTemplateRunPacketRequest` 和 `ApprovedTemplateRunPacket`
  metadata。
- 把 single-server profile、approved template、structured review request、
  workspace plan 和 lifecycle id 合成 non-executing run packet。
- 只渲染 allowlist 结构化参数，拒绝未批准参数、缺失必填参数、template/profile
  mismatch、workspace plan validation error 和用户自由 command。
...
- 不执行命令、不接受自由 command、不读取凭据、不连接服务器、不创建 workspace。

`v0.15 Internal Sanity-Check Runner Gate` 已在 2026-06-23 完成第一批实现：
...
- 新增 `SanityRunnerDefinition`、`SanityRunnerRequest` 和
  `SanityRunnerReviewPacket` metadata。
- 支持固定 allowlisted runner id review、timeout metadata、stdout/stderr capture plan、
  workspace-root artifact path preview 和 metadata-only audit event plan。
- 测试证明 unknown runner id、用户自由 command、删除请求、凭据读取、SSH、Slurm、
  PBS、remote server access、artifact path 逃逸和缺失 timeout/capture plan 都会被拒绝。
- 不执行命令、不读取凭据、不删除文件、不连接服务器、不接 SSH/Slurm/PBS、不创建
  workspace、不采集真实 stdout/stderr 或 artifact。
...
- 路线把 v0.11 到 v1.0 拆成 safe operations、fake lifecycle、workspace
  ...
  closeout；v0.11 到 v0.15 第一批 metadata gates 已完成。
- 新窗口复制提示词和详细 agent 执行计划保存在本地忽略文件中，不再提交到 GitHub。

`v1.0 internal preview` 已在 2026-06-23 完成收口：
...
- 审计确认 v0.11-v0.15 的安全边界和全量 CTest 均满足。
- 内部预览可以演示 approved template、结构化参数、review packet、
  fake lifecycle、workspace/log/artifact preview 和 sanity runner gate。
- 这仍是 review-only internal preview，不启用真实服务器执行、凭据读取、
  SSH/Slurm/PBS、任意 shell、真实删除、workspace 创建或 artifact 采集。

以上结论描述 2026-06 的通用实验室后端路线。当前新增的 Deepwave CPU/单 GPU FWI
采用独立固定白名单，不代表通用 CUDA/MPI、Slurm、PBS、SSH 或实验室服务器后端已经
启用；这些通用能力仍保留到后续经过评审的里程碑。

## Historical New Conversation Workflow
...
These historical rules still apply to the generic server/backend path. The
current Deepwave CPU/single-GPU demo is a separate fixed-whitelist path and
does not relax them.

- Do not execute arbitrary CUDA/MPI jobs through the generic backend; only the
  reviewed fixed Deepwave demo path may launch its constrained worker.
...
- v0.13 Workspace Planner has completed preview-only workspace/run/log/artifact
  planning and path-safety validation.
- v0.14 Approved Template Run Packet has completed non-executing packet
  rendering from approved templates, structured parameters, workspace plans,
  lifecycle ids, and explicit execution-disabled flags.
- v0.15 Internal Sanity-Check Runner Gate has completed fixed allowlisted
  runner review packet metadata and tests without enabling execution.
- v1.0 internal preview is complete for the review-only single-server workflow:
  audit, user guide, operator runbook, demo script, consolidated test report,
  and learning summary are available.

Next session should either use the v1.0 internal preview docs for study/demo or
continue the later M11 real backend approval path. Do not execute commands, read
credentials, create directories, delete files, move files, or connect real
