...
> 历史版本说明：本文只描述 2026-06 的 v0.14 通用 run-packet 模块。当前仓库另有
> 固定白名单 Deepwave CPU/单 GPU FWI 执行路径；通用 JobBackend 仍保持本文所述的
> 非执行边界。当前能力以 [根 README](../../README.md) 为准。
...
- v0.14 通用 run-packet 本身不执行 CUDA/MPI；这不描述后来加入的固定 Deepwave FWI 路径。
