...
- 谁批准了这个选择，批准记录在哪里。
- 凭据放在哪里，代码只能引用凭据名称或密钥引用，不能保存密码、token、
  私钥或集群账号。
...
这些信息没拿到之前，代码层面只能继续做文档、评审预览、dry-run 包和学习总结。
...
- 本地开发环境如何处理：默认不接真实凭据，只使用 dry-run 或假的密钥引用。
...
只有模板和参数范围明确后，才能考虑 M11-T4 的提交、状态轮询和取消。
...
- 每个作业记录哪些字段：请求 ID、作业 ID、用户、后端、模板、参数摘要、
...
代码里现在只有“仅 metadata 的审计事件”和“内存审计日志”。生产审计存储是
...
- 第一个已批准作业模板是哪个算法：FWI、frequency extrapolation、post-stack
  inversion，还是更小的 sanity-check 作业？
- 允许哪些参数，参数范围是什么？
- 禁止用户传哪些字段？
- dry-run 提交包需要给 operator 看哪些内容才算可批准？

### 监控和审计

- 调度器或 wrapper 的作业 ID 如何获取？
- 状态轮询频率和超时时间是多少？
...
拿到 M11-T1 决策包以后，不要马上改运行时守卫。后续代码顺序应该是：
...
4. 再实现 M11-T5：日志收集和 artifact 索引。测试必须覆盖 artifact 缺失和异常日志。
...
- `docs/upgrade/single-server-backend-v0.10.md`：记录“一个服务器账号、自己先跑”
  的单服务器接入准备设计。
- `docs/upgrade/v1.0-internal-preview-roadmap.md`：记录内部预览的分步路线。
- `docs/upgrade/test-report-m11-preflight.md`：记录“仅 metadata 预检”的测试证据。
...
- `docs/upgrade/test-report-v0.9.md`：记录“非执行后端就绪评审”的测试证据。
- `docs/upgrade/learning-summary-v0.9.md`：解释就绪报告、提交包、审计预览和
  workspace/artifact 计划。
...
上一阶段已经完成了 M11 预检和 v0.9 后端就绪评审。它们能证明系统
可以用结构化 metadata 表达审批、授权、workspace、模板和审计，也能把这些信息渲染
成 operator 可以评审的文本。但这些文档仍然偏“代码侧”和“产品侧”，还缺一份
面向真实实验室推进的流程指南。
...
event/log validation 和就绪状态渲染。后续 M11-T2 到 M11-T7 应该继续围绕这个
边界扩展测试。
...
项目已有 `BackendApprovalDecision`、`BackendPreflightPackage` 和就绪状态渲染。
我补的是组织流程到工程实现之间的桥：实验室必须先给出后端决策包，
代码再把它映射为 metadata validation，然后按 M11-T2 到 M11-T7 的顺序实现认证、
workspace、提交/状态/取消、artifact 收集、可视化和审计。
关键点是运行时守卫不因 metadata 完整就打开，真实后端只在全部控制和测试存在后
才允许被启用。
