...
有界永久删除、六张标准图片、P2-006 无浏览器再派发的受监督调度与持续状态追赶，以及
P2-007 exact-attempt user cancellation 和 P2-008 exact-attempt wall-time timeout，再可选检查旧
MCP/FWI Result 兼容性，最后一键关闭。
...
- session scope 与 capability；当前应精确为
  `supervised_runtime_scheduling=true`、`continuous_status_supervision=true`、
  `supervisor_leases=true`、`features.running_cancel=true`、`capabilities.cancel=true`，而
  `startup_dispatch_recovery=false`、
  `startup_receipt_recovery=false`、`startup_status_catchup=false`、
  `automatic_reconciliation=false`；三个 startup 字段为 false 表示取得 lease 前的 inventory
  完全只读，不是表示 active-term scheduler 缺失；
...
1. approve/submit 成功回应允许 dispatch 为 `pending`、task 为 `Queued`；这表示已原子
   入队，不表示 HTTP Handler 已调 Adapter。页面随后只做 GET 轮询，active Supervisor
   将其推进为 `dispatching`/`dispatched` 与 `Queued`/`Running`/`Succeeded`；任务较快时
   可能看不到每个中间帧，但浏览器不 POST 再派发，终态也不会被伪造；
...
P2-006 启用后，页面 GET 既不是首次派发触发器，也不再是运行中 task 状态进入
SQLite 的唯一触发源。可在 approve 已返回 `pending` 后关闭任务卡或停止页面轮询，
等待超过 Supervisor poll 周期后再重开：active term 应完成当前 1.4 的首次调度，
并把后续状态写入 SQLite。approve/submit 本身仍是 enqueue-only，这个检查不应通过页面
二次 POST 来“帮助”调度。
...
### 5.5 验收 exact-attempt 运行中取消

选择一个运行时间足以观察到 `Running` 的 current 1.4 托管任务并打开详情。左栏列表保持
SQLite-only，故摘要固定 `can_cancel=false`；详情页才对 exact Adapter capability 做只读 probe。
只有 private schema 1.1、durable dispatched、latest spawned+ready+running attempt 且 Worker 已
发布 capability 时才出现 **取消任务**。pending/staged、legacy private schema 1.0 和公共
Adapter 1.0–1.3 不应出现可用取消按钮。

点击 **取消任务** 并确认。预期：

1. 浏览器 POST body 精确为 `{"reason":"user_requested"}`，带同源 Origin、CSRF 和稳定
   Idempotency-Key；关闭卡片、移除对话引用、放弃草稿和 Trash 都不会发送这个请求；
2. 首次成功响应显示 cancellation `requested`，Task 仍为 `Queued` 或 `Running`，按钮变为
   “取消处理中”；requested 绝不能显示成已取消；
3. active Supervisor 投递 exact request；Worker 自行 ack/unwind，不由控制面根据持久 PID 发 kill。
   只有 ack、`stopped` heartbeat 和 idle execution `flock` 全部成立后，Task 才变为 `Cancelled`，
   cancellation 变为 `cancelled` 并出现解决时间；
4. 若 Worker 先自然 `Succeeded`/`Failed`，原终态保持，cancellation 显示 `superseded`；
5. 若连接在响应前中断，页面只用 GET 审计，保留原 key 且不自动 POST；只有用户点击
   **继续取消（复用原 Idempotency-Key）** 才显式重放。

本节只验收用户取消；自动 timeout 另按下一节验收，二者不得混成同一种终态。

### 5.5A 验收 exact-attempt wall-time timeout

选择 current 1.4/private 1.1 的托管任务，并使用足以观察到运行中的已批准
`resources.wall_time_seconds`。预期：

1. 页面没有 timeout mutation 按钮，服务端也不存在 `POST /timeout` 路由；active Supervisor 只有
   在 exact Worker 发布 v2 exact-stop capability 后才建立计时窗口；
2. 详情展示的计时起点精确为 Store 对该 current Worker 的首条 SQLite durable
   `spawned + ready + running` observation 的 `observed_at`，截止时间为该时间加已批准 wall time；
   旧任务或 capability 不足时只显示“尚无运行时计时证据”；
3. deadline 前 timeout 为 `armed`，仍可由用户正常取消；deadline 后变为 `requested` 时 Task 仍是
   `Queued`/`Running`。deadline 到来且 active term 持久化 authorization 前 Worker mutation 为零；
   浏览器 GET 只读且不得把它提前显示为 Failed；
4. exact Worker 自行 ack/unwind；grace 耗尽时由该进程自退出，控制面不根据持久 PID 发 signal。
   只有 request、ack、`stopped` heartbeat 与 idle execution `flock` 全部成立，SQLite Task 才变为
   `Failed`，timeout 为 `timed_out` 且 failure code 为 `WALL_TIME_EXCEEDED`；
5. 自然 `Succeeded`/普通 `Failed` 在 timeout authorization 前完成时显示 `not_triggered` 并保留
   原终态；authorization/request 后自然终态先赢时显示 `superseded` 并保留原终态；
6. durable user-cancel admission 先于 timeout delivery authorization 时 timeout 显示 `suppressed`。
   之后仍按 P2-007 竞争：完整取消 proof 成立才是 `Cancelled`；若自然 `Succeeded`/`Failed` 先到，
   cancellation 为 `superseded` 并保留原终态；
7. Workbench timeout object 精确只有 `state`、`wall_time_seconds`、`started_at`、`deadline_at`、
   `resolved_at`、`failure_code`、`terminal_status` 七个只读字段，不得出现 ID/hash/PID/path。

该边界的最终自动化证据为 Web Python 46/46 与 Node UI PASS；完整 P2-008 checkpoint 另有 Runtime
304/304、固定 venv Worker 32/32（CPU smoke 3/3）、launch-control 25/25、Embedding 6/6、CTest
39/39、MCP 1/1、governance 和真实固定 venv CPU Deepwave timeout E2E PASS；未运行 CUDA。
...
事件和 artifact 仍可查询。当前启动在取得 lease 前只做有界 scope-bound 只读
inventory：不调 Adapter、不收养 receipt、不刷新 status、不写 runtime 状态。随后必须由
active fenced Supervisor term 完成调度/投影；另一存活 Web owner 已持有该 scope 时，
新 Web 应在提供 API 前失败。

为验收 P2-006 重启恢复，应分清以下边界：

- 当前 Adapter identity 1.4 的 `pending` 和 `dispatching`/no-record 可由 active term 调度；
- 当前 private schema 1.1 的 exact `preparing` 或 `launching` record 只在同一 attempt 仍为
  `staged`、无 slot/PID/ready/heartbeat、配置与 fingerprint 完全匹配时 resume；已有
  启动证据时只投影/收养，不二次 `Popen`；
- 当前 Adapter identity 1.4 下、早于 managed Worker evidence 的 legacy private schema
  1.0 exact `launched` receipt，可在 request/plan/fingerprint 与 receipt record hash 均精确
  证明后 fenced adoption；这不等于历史公共 Adapter identity 1.0–1.3，后者仍只读
  deferred，不可由当前 scheduler 新派发或收养。

任意 partial/inconsistent staging、无法精确绑定的证据与已有 `reconciliation_required`
仍 fail closed。SQLite term 只授权控制面；Worker 继承的 run/capacity kernel `flock`
才是执行与容量权威，heartbeat 不论新鲜或过期都不单独授权 takeover。这也不代表
partial staging 修复、reconciliation 解决、通用有限 task retry 或 SSE 已实现；
容量不足后对同一 pre-Popen staged attempt 的安全继续不是新 task retry。

P2-007 的取消边界不改变上述重启/调度规则：只处理 exact current managed Worker；取消请求
持久化后由 active term 接续投递和解决，不能按 heartbeat TTL 替换 Worker，也不能靠扫描
`FWI_RUN_ROOT` 恢复请求。P2-008 的 timeout 同样只处理具备 v2 exact-stop capability 的 exact
current managed Worker；它已验证但不表示 partial staging 修复、reconciliation 解决、通用有限
task retry 或 SSE 已实现。
...
历史 P2-005A checkpoint 的 Web Python 基线为 45/45 PASS；P2-006–P2-008 当前分支以上述命令
验证 Web Python 46/46 PASS，UI Node 输出 `ui message rendering tests passed`。这组测试同时证明
执行型文本先进入 Guided、纯理论文本仍
走聊天、旧结果不会把无合法回执的说明或代码误标成已提交，并覆盖旧 artifact 路径/后缀边界，
以及 Web 的 lease-before-listen、Supervisor 自我隔离、信号清理、零请求关闭、bounded drain、
严格 cancel body/CSRF/idempotency、requested 非终态、superseded 与 unknown-response 原 key 重放，
以及 timeout 七字段白名单、四态竞争、requested 非终态与不存在 timeout mutation。
...
Worker。已成功 exec 的 Worker 独立继承 run/capacity kernel `flock`，Web/Supervisor lease
失效不会释放这些执行权威；后继 term 只能根据精确 Worker 证据收养，不能因 heartbeat
过期就接管或二次启动。
