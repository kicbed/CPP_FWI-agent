...
P2-007 exact-attempt user cancellation，再可选检查旧 MCP/FWI
Result 兼容性，最后一键关闭。
...
本节不验收 timeout。`resources.wall_time_seconds` 只是持久资源策略字段，不是 runtime timeout；
timeout 的终态语义、计时起点和 force policy 仍 Pending。
...
partial staging 修复、reconciliation 解决、timeout、通用有限 task retry 或 SSE 已实现；
...
历史 P2-005A checkpoint 的 Web Python 基线为 45/45 PASS；P2-006/P2-007 当前分支以上述命令
