...
有界永久删除、六张标准图片及 P2-006 无浏览器再派发的受监督调度与持续状态
追赶，再可选检查旧 MCP/FWI
Result 兼容性，最后一键关闭。
...
- session scope 与 capability；当前应精确为
  ...
  `supervisor_leases=true`，而 `startup_dispatch_recovery=false`、
...
partial staging 修复、reconciliation 解决、cancel、timeout、通用有限 task retry 或 SSE 已实现；
...
历史 P2-005A checkpoint 的 Web Python 基线为 45/45 PASS；P2-006 当前分支以上述命令验证
Web Python 46/46 PASS，UI Node 输出 `ui message rendering tests passed`。这组测试同时证明
