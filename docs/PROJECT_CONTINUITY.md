...
- 决策状态：**Accepted**
- 实现状态：**P0 contracts + P1.1a Task Store + P1.1b Registry + P1.2a fixed Deepwave Adapter Verified / P1 in progress**
- 用户确认日期：2026-07-15
...
- 尚未实现：批准预算消费、Gate + approval + durable submit intent + `Queued` 同事务、事务后
  dispatch/reconciliation、审批/任务 API、Guided Web、取消/恢复调度、DAG、Agent Planner、
  通用 Algorithm SDK 和去噪链路。
