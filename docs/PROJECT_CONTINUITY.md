...
用户在 2026-07-17 明确要求：历史 `D-001`～`D-012` 既往不究并保持原状；从本规则生效后，
任何 Codex 都不得擅自新增、删除、重编号、重排或修改编号 `D-*` 条目。修改包括标题、状态、
日期、方向、范围、依据、实现/验证状态及正文任意字符；削弱本锁或更新受保护快照 hash 也属于
同一类受控变更。本节不是新的编号决定，也不改变任何既有 D 内容。

如确需变更，Codex 必须先保持工作树不变，展示只涉及一个目标（`D-NNN` 或 `D-LOCK`）、一个
操作的完整精确 diff 及其 SHA-256；hash 对象是 diff 代码块围栏内的精确 UTF-8 字节（不含围栏，
LF 换行，末尾一个 LF）。
随后给出一条已经填完整的授权句并结束回合。授权句固定格式为：

`我原样确认 D-AUTH-<唯一编号>：仅授权按已展示的补丁 SHA-256=<64位哈希>，对 <D编号或 D-LOCK> 执行 <新增/删除/重编号/重排/修改>；本授权仅此一次，不授权任何其他 D-* 变更。`

只有用户在同一对话的后续消息中，单独、原样复制 Codex 给出的完整实值句子才构成一次授权；
除传输首尾空白外，增删或改变任何字符都无效。“同意”“继续”“固定”“记录”“修正”、对方案
内容的认可、旧消息或其他窗口的文字均不授权 D 变更。授权只覆盖该 diff hash、D 编号和操作，
用后即失效；提案变化或上下文丢失必须重新展示并取得新的原样复制。一次授权不得覆盖多个 D。
已展示 diff 可包含该单一目标必需的契约/hash 与非 D 镜像更新，但所有字节都须进入同一 hash，
不得借机修改另一 D 或 `D-LOCK`；削弱授权协议本身时，唯一目标写 `D-LOCK`。

没有有效授权句时，普通实现和验证进度只写 `docs/PROJECT_PROGRESS.md`，不得触碰受保护 D 正文。
完整 D-003 固定经过 P0→P1→P2→P3→P4→P5→P6，只有通过 P6 出口才完成，P5 绝非项目终点。
若 D 正文或 `D-LOCK` 与受保护 hash 不一致，Codex 必须停止所有 D 写入并报告具体目标/diff；
不得自动修复、回滚或更新基线，既有不一致不构成授权，修复也必须重新取得原样 `D-AUTH`。
不得新增更深层的 `AGENTS.md` 或任何 `AGENTS.override.md` 来覆盖根锁；改变该边界同样以
`D-LOCK` 为唯一目标并完成上述原样授权。
...
- 决策状态：**Accepted**
- 实现状态：**精简接续治理契约 Implemented / Verified；运行时阶段不变**
- 用户确认日期：首次确认 2026-07-16；精简接续修订 2026-07-17

### 已采纳方向

1. 保留 D-003 的阶段顺序、可靠性、安全边界和出口质量，继续按可审阅、可验证的切片逐步
   开发；减少的是不必要的切片数量，不是测试覆盖或功能质量。
2. 区分“路线切片”和“工作轮次”：路线切片仍是弹性的产品交付/估算单位；工作轮次是一次
   Codex 请求内的有界执行单位，只含一个主要验收目标和一个风险边界，可在路线切片仍
   `In progress` 时安全交接，不新增路线编号，也不改变滚动余量。
3. 用户只说“继续 D-003”时，默认只执行一个工作轮次，而不是承诺一次完成整个路线切片或阶段。
   动手前用简短工作包声明目标、风险边界、预计触及面、验证层级和停止点；发现新的独立风险边界
   时停在安全交接点并记录下一动作，不静默扩大本轮。
4. 路线切片数量只是现场规划基线，不是固定配额。相邻工作共享同一状态机、接口、风险边界和
   出口测试时应合并；新增路线切片仍须先说明具体证据并获用户明确批准。不得为单个 migration、
   字段、receipt/sidecar 或一项测试单独创建路线切片，也不得把多个独立风险边界强塞进一轮。
   `0cbe131` 的约 12 个粗估锚点覆盖当时完整 P2 余项与 P3–P6，不是当前余量或配额。
5. 多算法接入表示用户在 Catalog 中看到多个兼容的独立算法选项，可上传/选择受控数据、描述
   任务、获得建议并选择其中一个执行。自动把多个算法串成端到端处理流水线不是当前验收要求；
   DAG 能力保留，但多节点执行必须由用户明确选择的计划或工作流触发。
6. 新算法的目标接入面接近 MCP 工具：功能描述、参数和 I/O Schema/Profile、固定 Adapter 或
   RPC 入口、资源/权限/环境声明、fixture 与 conformance test。已有 Profile 范围内的算法应为
   薄插件；只有首次引入新数据域或结果类型时才扩展公共 Profile/展示组件。
7. 验证固定为三级：工作轮次内只跑受影响/失败测试；到路线切片出口时，在候选最终 tree 上只跑
   一次相关集成 aggregate；只有阶段出口才跑完整回归和代表性 CPU/CUDA/E2E。候选 tree 后续变化
   时先补受影响测试，最终 aggregate 只对新的最终 tree 再跑一次。共享公共执行/安全合同确实变化
   时可扩大到被其影响的 suite，但必须先说明触发证据，不自动扩大为全仓回归。数值 Worker、依赖、
   数据和规范化配置身份不变时复用绑定 Git tree/环境/数据/配置的既有实验，不重复耗时实验。
8. 每轮默认一次综合审阅，不默认并行或连续多轮独立审计；只有具体高风险触发或上一轮未解决发现
   才增加第二轮，第三轮及以后必须先获用户明确批准。测试输出保持有界；aggregate 只报告一次，
   已包含的子集不得再次作为可相加的独立总量罗列。
9. `docs/PROJECT_PROGRESS.md` 是执行状态、验证证据和滚动余量唯一真源；短入口只保存路由、当前
   增量和不可破坏边界，并同时受 80 行与 8192 字节上限约束。计划只保存阶段/出口，决策只保存
   已采纳方向；不得在多份文档复制完整实现史、测试日志或相同状态矩阵。
10. 每个 Verified 交付在同一 checkpoint 按
...
- “继续 D-003”依次执行：最小读取与现场核对、声明本轮工作包、实现、受影响测试、综合审阅、
  有界记录和安全交接。未到路线切片出口时标记真实 `In progress`，不得为追求 `Verified` 擅自扩大。
- 到路线切片出口才运行一次相关 aggregate 并形成 Verified checkpoint；到阶段出口才运行完整
  回归与适用的代表性 CPU/CUDA/E2E。若扩大验证，先说明具体失效证据和受影响 suite。
- 工作轮次不是新增路线切片，不进入剩余数量公式；同一路线切片可跨多个轮次自然完成，用户只需
  继续说“继续 D-003”，无需记忆测试命令或内部子步骤。
...
8. 非 D 固定内容写适当计划/账本；任何 D 变更先展示单 D diff/hash，等用户原样复制 D-AUTH
...
| 2026-07-17 | D-011 | 永久改为有界工作轮次默认接续；阶段与质量门不变 | 用户要求“继续 D-003”不再默认一次完成整个路线切片，并要求该精简语义覆盖后续 P3–P6；采用轮次内聚焦测试、切片出口单次 aggregate、阶段出口完整回归/代表性 CPU-CUDA、审计轮次门和短入口双上限 |
