...
- 决策状态：**Accepted**
- 实现状态：**P0 contracts + P1.1a Task Store + P1.1b Registry foundation Verified / P1 in progress**
- 用户确认日期：2026-07-15
...
- P1.1b 已验证：SQLite v2 连续/原子 migration、不可变 Dataset Catalog/Algorithm Registry、
  ...
  33 项 TaskService 与 28 项合同测试通过。注册/allowlist 不代表 Adapter ready，`Queued` 仍关闭。
- 尚未实现：批准预算消费、Gate + approval + submit idempotency + `Queued` 同事务、审批/任务
  API、Deepwave 标准 Adapter、Guided Web、取消/恢复调度、DAG、Agent Planner、通用 Algorithm
  SDK 和去噪链路。
