...
- 决策状态：**Accepted**
- 实现状态：**P0 + P1 最小持久垂直切片 Verified / P2.1–P2.8 有界切片 Verified /
  完整 P2 Pending**
- 用户确认日期：2026-07-15
...
3. 不为单个 migration、Schema 字段、receipt/sidecar 变化或一项测试单独创建产品路线切片；
   也不为追求较小数字把不可独立验证的大量高风险工作强塞进一个提交。剩余工作允许比当前
   约十余个中等切片的估算稍多，但未经新证据与说明不得膨胀成几十个路线切片。
...
- 决策状态：**Accepted**
- 实现状态：**P2-009B In progress；P2-009B1 首次启动失败重试 Verified；P2-009B2 运行后退出重试 Pending**
- 用户确认日期：2026-07-17
...
- P2-009B2 仍 Pending；它必须先统一迁移 effective handle、status、artifact、cancel 与 timeout，
  不能从 B1 已验证状态推断 post-ready `worker_exit` 已实现。
