...
- 决策状态：**Accepted**
- 实现状态：**P0 contracts Verified / Durable runtime pending**
- 用户确认日期：2026-07-15
...
- P0 已实现并验证：只覆盖最小 Marmousi/Deepwave FWI 链路的七类 Draft-07 JSON Schema、
  ...
  Adapter/Proto 规范、威胁模型和旧合同差异审计。23 项合同测试及现有 C++/FWI/Web 回归通过。
- P0 合同和 Gate 参考实现不持久化、不调度、不提交 Worker，不能冒充 TaskService。
- 尚未实现：P1–P6 的 SQLite TaskService、Dataset Catalog 注册、审批 API、Deepwave 标准
  Adapter、Guided Web、取消/恢复调度、DAG、Agent Planner、通用 Algorithm SDK 和去噪链路。
