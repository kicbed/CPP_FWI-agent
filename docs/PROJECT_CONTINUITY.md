...
- 实现状态：**P0 + P1 最小持久垂直切片 Verified / P2.1 有界切片 Verified /
  P2.2 有界回收站 Verified / 完整 P2 Pending**
- 用户确认日期：2026-07-15
...
- 尚未实现：P2 cancel/timeout/lease/heartbeat/retry/自动 reconciliation/SSE，以及
  ...
  链路也仍 Pending。服务器 transcript 永久删除和 artifact 永久 purge 也尚未实现；
  P2-001/P2.2 均不得被报告为完整 P2 已实现。
...
4. 保留对话删除能力并增加明确确认。本轮只安全删除当前浏览器中的对话副本；服务器 transcript
   ...
   已从服务器永久清除。
5. 用户“删除任务”采用可恢复 Trash/Restore，而不物理删除不可变 Draft、Plan、Approval、
   ...
   对话删除和任务回收站操作绝不级联。
6. 永久 artifact purge 是后续独立 retention 能力，必须先有 tombstone、运行中拒绝、崩溃恢复
