...
14. P2.8 wall-time timeout 采用三项明确语义：终态固定为 `Failed / WALL_TIME_EXCEEDED`，不得
    ...
    该能力只对能证明 v2 exact-stop capability 的 current 1.4/private 1.1 managed attempt 开放；
    ...
    Supervisor 尚未投递而被后来到期的 timeout 重分类。
...
- P2-007 exact-attempt user cancellation 已验证：SQLite v11 request/authorization/outcome 与
  immutable trigger 把请求绑定到 current Algorithm/Adapter 1.4、private schema 1.1、dispatched
...
- P2-008 exact-attempt wall-time timeout 已验证：SQLite v12 immutable window、active-term
  authorization 与 outcome 只覆盖 current 1.4/private 1.1、durable dispatched、latest managed
  ...
  Node/governance 及真实固定 venv CPU Deepwave timeout E2E 均 PASS；未运行 CUDA。
- 尚未实现：有限 retry、已有 `reconciliation_required` resolution、SSE 和完整自动
  ...
  transcript 永久删除、SQLite 审计历史硬删除以及备份/外部副本清除仍未实现；P2-001–P2-008
  均不得被报告为完整 P2 已实现。
...
2. 切片数量只是现场规划基线，不是固定配额。相邻工作共享同一状态机、接口、风险边界和出口
   ...
   风险，可以合理增加切片并在进度账本记录依据。
3. 不为单个 migration、Schema 字段、receipt/sidecar 变化或一项测试单独创建产品路线切片；
   也不为追求较小数字把不可独立验证的大量高风险工作强塞进一个提交。“约十余个”是从当前
   到整个 D-003 完成（完整 P2、P3、P4、P5）的粗估，不是任一 P 级别内的数量或固定配额。
...
- 该决定不把“约十余个”升级成承诺数字，也不预先批准跳过依赖；现场代码和风险证据决定是否
  合并或拆分。
...
## D-013：估算范围标注与已接受计划变更控制

- 决策状态：**Accepted**
- 实现状态：**Governance Implemented → Verified；运行时阶段不变**
- 用户确认日期：2026-07-17

### 已采纳方向

1. 任何“剩余多少切片、多久、多少工作项”等数量或工期估算，都必须同时标明：估算对象是
   整个项目、某个 P 阶段还是某个子切片；包含哪些阶段；以哪个当前 checkpoint 为基线；以及
   它是可随现场证据调整的粗估，还是用户明确接受的承诺。
2. 全项目估算与当前阶段子集必须并列区分，不能用一个阶段的收敛结果覆盖全项目范围。例如
   当前“约十余个”覆盖从当前到整个 D-003 完成（完整 P2 与 P3–P5），而“两个”只指 P2 收尾。
3. 已标记 Accepted 或用户要求“固定”的计划、范围、阶段顺序、依赖、安全边界和退出标准，未经
   用户再次明确同意不得修改、缩减、扩张、重新解释或用新估算暗中替换。现场进度与测试证据
   只能更新 Implemented/Verified/Pending，不构成改变 Accepted direction 的授权。
4. 如新证据确实建议改计划，先说明证据、收益、成本、风险和兼容性，明确请求用户批准；获批后
   才能修改，并在同一变更中保留旧决定、标注 superseded 关系和变更原因。

### 实现与验证状态

- 根 `AGENTS.md`、短入口、进度账本与架构计划均加入作用域标注和变更控制；continuity contract
  固定 D-013 及全项目/P2 两层估算文字。治理、launcher、Node UI 与 diff 检查通过。
...
8. 用户明确要求保留的决定自动写入本文；其他新建议先征得同意
...
| 2026-07-17 | D-011 | 明确“约十余个”的估算范围 | 用户确认该粗估覆盖从当前到整个 D-003 项目完成（完整 P2 与 P3–P5），不是一个 P 级别；当前“两个剩余切片”只指 P2 收尾，不减少后续阶段或阶段质量门 |
| 2026-07-17 | D-013 | 新增并标记 Accepted；治理实现与验证完成 | 用户要求以后所有此类估算加好范围标注，避免新窗口误解；此前固定/Accepted 计划未经用户明确同意不得修改或重释。自动入口、连续性真源和契约测试已固定该规则 |
