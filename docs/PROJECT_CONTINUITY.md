...
- 决策状态：**Accepted**
- 实现状态：**P0 + P1 最小持久垂直切片 Verified / P2.1 有界切片 Verified / 完整 P2 Pending**
- 用户确认日期：2026-07-15
...
- D-006/P1-006 已实现并验证：Guided/MCP/Worker 参数上限为 10000；该 checkpoint
  ...
  六参数历史快照；当前新提交使用 `1.3.0`，并保留 `1.0.0`/`1.1.0`/`1.2.0`
  任务和收据的严格读兼容。
...
- 尚未实现：P2 cancel/timeout/lease/heartbeat/retry/自动 reconciliation/SSE，以及
  不依赖用户点击的运行意图/事件流恢复；P3 DAG、Agent Planner、通用 Algorithm SDK 和去噪
  链路也仍 Pending。P2-001 不得被报告为完整 P2 已实现。
...
- D-006 checkpoint 的读取兼容强制 Algorithm/Adapter/fingerprint 只能是 `1.0↔1.0` 或
  ...
  快照保持不可变并继续严格读兼容；当前新 dispatch 使用严格的 `1.3↔1.3`，同时
  `1.0↔1.0`/`1.1↔1.1` 也保持读兼容。MCP runner 合同不因 Web 优化器控件而
  获得任意参数或 shell 能力。
...
- 已实现 Algorithm/Adapter `1.3.0`、合同 minor `1.1.0` 的六参数 Draft/Plan、推荐卡、
  受校验表单、滚动保持、SQLite v5 scope index、有界 cursor 列表、左栏和单任务重开。
  `1.2.0` 保留为不可变六参数历史快照，只用于严格读取既有任务和收据。
- 当前 `1.3.0` manifest 只声明 `acoustic_fwi_2d` 与 `fwi_smoke|fwi_demo`，iterations
  ...
  未命中历史 durable record 才确定性补为 Adam/LR 10 后按当前 `1.3.0` 校验和建模。同 key
...
- 当前自动化证据：Scientific Runtime 控制面 157/157、固定 venv Worker 27/27、Web
  Python 29/29、Node UI 行为和 `git diff --check` 全部 PASS。真实 HTTP 验证旧 500 次
  CPU/Algorithm 1.1 任务可从持久列表重开为 Succeeded 并读取两个 artifact；当前 1.3
