...
- 尚未实现：P2 Worker fenced capacity/attempt lease 与独立 heartbeat、分阶段 Worker 启动、
  pending/no-record 调度、cancel/timeout/retry、已有 `reconciliation_required` resolution、SSE 和
  完整自动 reconciliation。startup recovery 与浏览器 GET 仍可走无 Supervisor lease 的既有单调
  CAS，因此 P2-005A 也不表示所有状态写入由唯一控制面独占；P3 DAG、Agent Planner、通用
  Algorithm SDK 和去噪链路也仍 Pending。服务器
  transcript 永久删除、SQLite 审计历史硬删除以及备份/外部副本清除仍未实现；P2-001–P2-004
  与 P2-005A 均不得被报告为完整 P2 已实现。
