...
这个方向已记录为 `D-003 Accepted`。P0 已完成并验证七类最小 FWI Schema、规范化
plan hash、确定性 Gate、执行指纹、API/Adapter 草案和安全威胁模型；它们目前是合同和
无副作用参考实现，不会持久化任务或提交 Worker。P1–P6 仍为 `Pending`，所以下方的
“当前能做什么”仍是唯一已上线基线。

新 Codex 会话会自动读取进度账本并从 P1 的首个未完成切片继续；用户直接说“继续执行
D-003”即可，不需要记忆脚本或旧的 P0.1 启动口令。P1 将先实现 SQLite WAL 最小
TaskService，不会跳到 Web、P4 Agent Planner 或未经批准的 FWI 计算。
