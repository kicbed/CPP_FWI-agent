...
## 下一代 Runtime：P0 + P1 已验证

`D-003 Accepted` 的 P1 最小持久垂直切片已完成：固定 Marmousi/Deepwave 任务从
Guided 表单进入 TaskDraft/Plan 确认卡，用户可修改、批准或放弃 pre-runtime 草稿；
批准后任务通过 SQLite TaskService、固定 Adapter 和 one-shot dispatch 实际运行，页面展示
真实状态、事件与受控 NPY/CSV 结果。

当前按要求停在 P2 之前。运行中取消、lease/heartbeat、task retry、自动
reconciliation、SSE、任务列表/页面刷新恢复仍为 Pending；页面的 pre-runtime “放弃”不是
运行中 cancel。P3 DAG 和 P4 Agent Planner 也未提前实现。
...
4. 成功后预期恰好两张标准结果卡：反演速度模型 NPY 与损失曲线 CSV。
...
超过 100 次的任务可能长时间占用 CPU/GPU；P1 尚不支持运行中取消，应先确认资源和参数。
